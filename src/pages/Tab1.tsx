...
import { checkmarkCircleOutline, alertCircleOutline, checkmarkDoneCircleOutline, closeCircleOutline, chevronForwardOutline, checkmarkOutline, closeOutline } from "ionicons/icons";

import "./Tab2.css";

import { Link } from "react-router-dom";
import firebase, { getCurrentUser } from "../firebaseConfig";
import { collection, where, query, onSnapshot, doc as task, writeBatch, getDocs } from 'firebase/firestore';
...
function WhichIcon(props: any) {
  ...
  return <IonAvatar slot="start"><IonIcon icon={status[e_status].icon} size="large" color={status[e_status].color} /></IonAvatar>
}
...
const Tab1: React.FC = () => {
  ...
  const updateStatus = (id: string, status: number) => {
    writeBatch(firestore).update(task(firestore, "tasks", id), { status }).commit().then(() => {
      getTasks();
    })
  }
  ...
  const getTasks = async () => { 
    ...
    const q = query(tasksRef, where("receiver", "==", user?.email), where("status", "==", 0));
    ...
    <IonPage>
      <IonHeader>
        <IonToolbar>
          ...
          <IonButton onClick={() => handleLogOutButton(auth)} slot={"end"}>Log out</IonButton>
        </IonToolbar>
      </IonHeader>
